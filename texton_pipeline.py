...
folder = r"E:\spatially_sorted_lvisf2_is2olvis1bcv\55745_to_55841\patches"
import numpy as np
...
def makeRFSfilters(radius=5, sigmas=[1, 2], n_orientations=6):
...
if __name__ == "__main__":
    ...
    dictionary = np.zeros((1640,61,61,8),dtype=int)
    for n in range(1641):
        patchfile = r"E:\spatially_sorted_lvisf2_is2olvis1bcv\all_patches\patch"+str(n)+".png"
        img = cv2.imread(patchfile, cv2.IMREAD_GRAYSCALE)
        ...
        for axes, res in zip(ax2.ravel(), response):
            ...
            # im.save(r'E:\spatially_sorted_lvisf2_is2olvis1bcv\all_patches\your_file.png')
            dictionary[n,:,:,m] = res
            m+=1
    ...
    df = pd.DataFrame(data=[(*axes, v.item()) for axes, v in zip(it.product(*[range(i) for i in dictionary.shape]), np.nditer(dictionary))], columns=('patch_number','x','y','filter'))
    df.to_csv(r"E:\spatially_sorted_lvisf2_is2olvis1bcv\all_patches\texton_filter_responses.csv")