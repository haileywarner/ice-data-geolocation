...
#             np.save(folder,'patch_r'+str(p)+str(q), res)
#             q+=1
...
patchfiles = glob.glob(folder+'\patch_r15*')
sum = np.zeros((513,513),dtype=float)
for patchfile in patchfiles:
    patch = cv2.imread(patchfile, cv2.IMREAD_GRAYSCALE)
    sum+=patch
mean = sum/6
plt.imshow(mean)
plt.show()